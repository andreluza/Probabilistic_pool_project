...
# observed richness
obsSR_1000 <- rowSums (data_1000_traps_var, na.rm=T)

# load pool richness
load(here ("data","pool_data","pool_probabilistico_add_especies.RData"))
potential_1000 <- apply (site_list_1000[[1]],1,sum) # 1000 trap-nights effort

# calculate deltaSR
deltaSR_1000 <- potential_1000 - obsSR_1000

# subset with more than 5 spp
min_spp <- 5
deltaSR_1000 <- deltaSR_1000[which(obsSR_1000 >= min_spp)]

# in a few cases deltaSR was lower than zero (likely due to species missing in the pool)
# set these values to zero
deltaSR_1000 [which (deltaSR_1000 < 0)] <- 0

# ----------------------------------------- #
#                  DeltaFD
# ----------------------------------------- #

#  1000 TRAPS (main text)

# FD observed for local communities
load(here ("output","hypervolume_obs_deltaFD_1000traps.RData"))

# null FD for each community
load(here ("output","hypervolume_local_null_deltaFD_1000traps.RData"))

# load pool FD (and then calculate average across iterations)
load(here ("output","hypervolumePOOL_1000traps_observed.RData"))
hypervolume_pool_obs_1000 <- lapply(hypervolume_pool_comm_1000,unlist)# melt the list
hypervolume_pool_obs_1000 <-do.call(cbind,hypervolume_pool_obs_1000) # melt the list
hypervolume_pool_obs_average_1000 <- apply(hypervolume_pool_obs_1000,1,mean,na.rm=T)# average pool hypervolume

# calculate observed deltaFD
deltaFD_1000 <- hypervolume_pool_obs_average_1000 - unlist(hypervolume_local_obs_1000)

# calculate null deltaFD
hypervolume_comm_null_1000 <- lapply(hypervolume_local_null_1000traps,unlist)# melt the list
hypervolume_comm_null_1000 <-do.call(rbind,hypervolume_comm_null_1000)

# pool FD minus local null FD
null_deltaFD_1000 <- lapply (seq (1,ncol (hypervolume_comm_null_1000)), function (it) # for each iteration
  
  hypervolume_pool_obs_1000 [,it] - hypervolume_comm_null_1000 [,it] # calculate the difference = deltaFD
  
)
null_deltaFD_1000 <- do.call(cbind,null_deltaFD_1000)# melt the list

# SES deltaFD
mean_deltaFD_1000 <- apply(null_deltaFD_1000,1,mean) # mean across iterations, per community
sd_deltaFD_1000<- apply(null_deltaFD_1000,1,sd) # standard deviation across iterations, per community

## standardized effect size
SES_deltaFD_1000 <- (deltaFD_1000 - mean_deltaFD_1000)/sd_deltaFD_1000

# data frame to analysis and plot
data_to_analysis_1000 <- data_1000_traps[which(obsSR_1000>=min_spp), # sites with 5 or more spp
                                       1:5] # covariates (coordinates and habitat)
data_to_analysis_1000 <- cbind (data_to_analysis_1000,
                               deltaSR = deltaSR_1000,# bind deltaSR
                               deltaFD = SES_deltaFD_1000# bind deltaFD
)
# create a factor to describe major type of habitat (natural or modified)
# (state of human modification)
data_to_analysis_1000$type <- data_to_analysis_1000$hab_cut_edge
# there is a NA in hab_cut_edge (it is a forest habitat as described in the rowName)
data_to_analysis_1000$type [which(is.na(data_to_analysis_1000$type))] <- "forest.0.0.0"
# adjust factor
levels (data_to_analysis_1000$type) [which (levels (data_to_analysis_1000$type) == "forest.0.0.0")] <- "Natural"
levels (data_to_analysis_1000$type) [which (levels (data_to_analysis_1000$type) == "grassland.0.0.0")] <- "Natural"
levels (data_to_analysis_1000$type) [which (levels (data_to_analysis_1000$type) != "Natural")] <- "Human-modified"

# changing the order habitat factor to have a good plot
data_to_analysis_1000$hab_cut_edge <- factor(data_to_analysis_1000$hab_cut_edge, levels = c("forest.0.0.0", "grassland.0.0.0",
                                                                        "open.0.0.0", "open.1.0.0","edge.0.0.0",
                                                                        "edge.0.1.0", "tree_plantation.0.0.0"))

# Figure 1 (main text)

pdf(here ("output","figures", "Fig2.pdf"),width = 4,height=4)
# scatter histogram
plot1000<- ggscatterhist(
      data_to_analysis_1000, x = "deltaSR", y = "deltaFD",
...
# lines
plot1000$sp <- plot1000$sp+ 
      geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
      geom_vline(xintercept = mean(data_to_analysis_1000$deltaSR), linetype = "dashed", color = "black") 
plot1000    
dev.off()   
...
mean(data_to_analysis_1000$deltaSR) 
sd(data_to_analysis_1000$deltaSR) # sd
# natural habtiat average
mean(data_to_analysis_1000$deltaSR [which(data_to_analysis_1000$type == "Natural")])
sd(data_to_analysis_1000$deltaSR [which(data_to_analysis_1000$type == "Natural")])
# human modified
mean(data_to_analysis_1000$deltaSR [which(data_to_analysis_1000$type == "Human-modified")])
sd(data_to_analysis_1000$deltaSR [which(data_to_analysis_1000$type == "Human-modified")])

# global average of deltaFD
mean(data_to_analysis_1000$deltaFD) 
sd(data_to_analysis_1000$deltaFD) # sd
# natural habtiat average
mean(data_to_analysis_1000$deltaFD [which(data_to_analysis_1000$type == "Natural")])
sd(data_to_analysis_1000$deltaFD [which(data_to_analysis_1000$type == "Natural")])
# human modified
mean(data_to_analysis_1000$deltaFD [which(data_to_analysis_1000$type == "Human-modified")])
sd(data_to_analysis_1000$deltaFD [which(data_to_analysis_1000$type == "Human-modified")])

#---------------------------------------
# linear regression (or anova) to test variation between / among factors
# major type (state of human modification)
summary(lm_1000_deltaSR <- (lm (deltaSR~type,data=data_to_analysis_1000))) # deltaSR
summary(lm_1000_deltaFD <- (lm (deltaFD~type,data=data_to_analysis_1000))) # deltaFD

#---------------------------------------
# differences between categories of habitat type
lm_1000_deltaSR_habitats <- (lm (deltaSR~hab_cut_edge,data=data_to_analysis_1000)) # deltaSR
lm_1000_deltaFD_habitats <- (lm (deltaFD~hab_cut_edge,data=data_to_analysis_1000)) # deltaFD
...
TukeyHSD (lm_1000_deltaFD_habitats_reaj, "hab_cut_edge", ordered=T)

#-----------------
# delta SR
# contrast plot
## extracting parameters 
nms <- unique (names (lm_1000_deltaSR_habitats_reaj$coefficients))
# adjusting names
...
df <- data.frame(term     = rep(nms, 1),
                 estimate = unname(lm_1000_deltaSR_habitats_reaj$coefficients))
# confidence intervals
df <- transform(df,
                lower = confint (lm_1000_deltaSR_habitats_reaj)[,1],
                upper = confint (lm_1000_deltaSR_habitats_reaj)[,2])
...
# contrast plot
## extracting parameters 
nms <- unique (names (lm_1000_deltaFD_habitats_reaj$coefficients))
# adjusting names
nms <- gsub ("hab_cut_edge","",nms)
nms <- sub("forest.0.0.0", "Forest", nms)
nms <- sub("grassland.0.0.0", "Grassland", nms)
...
# datafarme with estimates  
df <- data.frame(term     = rep(nms, 1),
                 estimate = unname(lm_1000_deltaFD_habitats_reaj$coefficients))
# confidence intervals
df <- transform(df,
                lower = confint (lm_1000_deltaFD_habitats_reaj)[,1],
                upper = confint (lm_1000_deltaFD_habitats_reaj)[,2])
# ordering factor levels
df$term <- factor(df$term, levels = c("Tree plantation",
                                      "Forest edge",
                                      "Grassland edge", 
...
pdf (here ("output","figures", "Fig3.pdf"),height=4,width=4,onefile=T)
p3_tukey_deltaSR
p3_tukey_deltaFD
dev.off()
...
# scenarios
data_to_analysis_1000 <- cbind (data_to_analysis_1000,
                                scenarioSR = ifelse (data_to_analysis_1000$deltaSR >=mean(data_to_analysis_1000$deltaSR),
                                                     "Higher-SR","Lower-SR"), # S1 and S3
                                scenarioFD = ifelse (data_to_analysis_1000$deltaFD >= 0,
                                                     "Higher-FD","Lower-FD")) # S2 and S4

# interaction to have scenarios
data_to_analysis_1000 <- cbind (data_to_analysis_1000, 
                                IntDeltaSR_FD=interaction(data_to_analysis_1000$scenarioSR,
                                                          data_to_analysis_1000$scenarioFD))

# edit levels of the factor 'scenarios'
data_to_analysis_1000$IntDeltaSR_FD <- factor (data_to_analysis_1000$IntDeltaSR_FD, 
...
with(data_to_analysis_1000, chisq.test (type,IntDeltaSR_FD,simulate.p.value = F))# to have degrees of freedom
with(data_to_analysis_1000, chisq.test (type,IntDeltaSR_FD,simulate.p.value = T))
with(data_to_analysis_1000, chisq.test (type,IntDeltaSR_FD,simulate.p.value = T))$expected
with(data_to_analysis_1000, chisq.test (type,IntDeltaSR_FD,simulate.p.value = T))$observed
...
sp_df_coord <- data_to_analysis_1000 [,c("LONG", "LAT")]
...
data_to_analysis_1000$biomes <- as.factor (overlap_pts_biomes$WWF_MHTNAM)
data_to_analysis_1000$biomes <-  droplevels(data_to_analysis_1000$biomes)
...
# transforming
levels (data_to_analysis_1000$biomes) [which(levels (data_to_analysis_1000$biomes) %in% tropical_forests)] <- "trop.for"
levels (data_to_analysis_1000$biomes) [which(levels (data_to_analysis_1000$biomes) %in% temperate_forests)] <- "temp.for"
levels (data_to_analysis_1000$biomes) [which(levels (data_to_analysis_1000$biomes) %in% tropical_grasslands)] <- "trop.grass"
levels (data_to_analysis_1000$biomes) [which(levels (data_to_analysis_1000$biomes) %in% temperate_grasslands)] <- "temp.grass"

# interaction between biome and state of human modification
data_to_analysis_1000$IntStateBiome <- interaction (data_to_analysis_1000$type,
                                                    data_to_analysis_1000$biomes)

# Chi-squared analysis
with (data_to_analysis_1000, chisq.test(IntStateBiome, 
                                        IntDeltaSR_FD,
...
# totals
rowSums(with (data_to_analysis_1000, chisq.test(IntStateBiome, 
                                                IntDeltaSR_FD,
                                                simulate.p.value = T))$observed
)
colSums(with (data_to_analysis_1000, chisq.test(IntStateBiome, 
                                                IntDeltaSR_FD,
                                                simulate.p.value = T))$observed
)
with (data_to_analysis_1000, chisq.test(IntStateBiome, 
...
wm_b <- wm + 
  geom_point (data=data_to_analysis_1000 [which(data_to_analysis_1000$type == "Human-modified"),], 
              aes (x=LONG, y=LAT,
                    colour=data_to_analysis_1000$IntDeltaSR_FD[which(data_to_analysis_1000$type == "Human-modified")]),
              size=1.7,shape=1, stroke = 0.9, position = jitter,
              alpha = ifelse(as.numeric(data_to_analysis_1000$IntDeltaSR_FD[which(data_to_analysis_1000$type == "Human-modified")])>=3,
                             1,0.7)) + #
...
wm_b2 <- wm + 
  geom_point (data=data_to_analysis_1000 [which(data_to_analysis_1000$type == "Natural"),], 
              aes (x=LONG, y=LAT,
                   colour=data_to_analysis_1000$IntDeltaSR_FD[which(data_to_analysis_1000$type == "Natural")]),
              size=1.7,shape=1, stroke = 0.9, position = jitter,
              alpha = ifelse(as.numeric(data_to_analysis_1000$IntDeltaSR_FD[which(data_to_analysis_1000$type == "Natural")])>=3,
                             1,0.7)) + #
...
crit_deltaSR <- seq (range(data_to_analysis_1000$deltaSR)[1], range(data_to_analysis_1000$deltaSR)[2], 0.5)
...
delta_SR_crit <- lapply (crit_deltaSR, function (threshold) {
  ...
  (table (data_to_analysis_1000$type [which (data_to_analysis_1000$deltaSR >  threshold)]))#/length(lista_DF[[i]]$DeltaSR)
...
crit_deltaFD <- seq (range(data_to_analysis_1000$deltaFD)[1], range(data_to_analysis_1000$deltaFD)[2], 0.05)

# the number of natural and human modified habitats per threshold
delta_FD_crit <- lapply (crit_deltaFD, function (threshold) {
  
  (table (data_to_analysis_1000$type [which (data_to_analysis_1000$deltaFD >  threshold)]))#/length(lista_DF[[i]]$DeltaFD)
...
png(file=here("output","figures","CumDenFunc_FigS5_1_1000T.png"), width=22, height=12, units="cm", res=600, family="serif")
...
how_many_to_get <- table(data_to_analysis_1000$type)[2] # pegar o numero referente ao numero de huamn modifeid
modified_habitats <- data_to_analysis_1000 [which(data_to_analysis_1000$type == "Human-modified"),] ## quantas florestas pegar, ja que tem muito mais floresta do que campo
# forest sites ( as we have a lot of them)
forest_sites_to_get <- data_to_analysis_1000[which(data_to_analysis_1000$hab_cut_edge == "forest.0.0.0"),]
# number of grassland sites
campos <- data_to_analysis_1000 [which(data_to_analysis_1000$hab_cut_edge == "grassland.0.0.0"),]
...
png(file=here ("output","figures", "CumDenFunc_uncert_FigS5_2_1000T.png"), width=22, height=12, units="cm", res=600, family="serif")
par(mfrow=c(1,2),mar=c(5,5,2,0.7))
